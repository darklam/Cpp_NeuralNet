...
Network *n, std::vector<std::vector<double>> inputs, std::vector<std::vector<double>> outputs){
  ...
  for(int i = 0; i < size; i++){
    ...
    this->fitnesses.push_back(this->chromosomes[i]->getFitness(this->in, this->out, this->n));
...
bool Population::nextGen(){
  if(this->currentGen < this->maxGen){
    ...
    double max, prevMax = this->best->getFitness(this->in, this->out, this->n);
